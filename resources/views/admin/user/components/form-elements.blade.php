...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('job'), 'has-success': fields.job && fields.job.valid }">
    <label for="job" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.user.columns.job') }}</label>
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" v-model="form.job" v-validate="''" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('job'), 'form-control-success': fields.job && fields.job.valid}" id="job" name="job" placeholder="{{ trans('admin.user.columns.job') }}">
