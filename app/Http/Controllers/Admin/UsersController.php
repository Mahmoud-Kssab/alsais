...
class UsersController extends Controller
{
    ...
    public function index(IndexUser $request)
    {
        ...
        $data = AdminListing::create(User::class)->processRequestAndGet(
            ...
            ['activated', 'avatar', 'email', 'email_verified_at', 'id', 'job', 'name', 'phone', 'uuid'],

            // set columns to searchIn
            ['address', 'avatar', 'email', 'id', 'job', 'name', 'phone', 'uuid']
        );

        foreach ($data as $item){
            $item->avatar = $item->getThumbs200ForCollection('avatar');
        }
